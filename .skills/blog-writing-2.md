...
- **이미지 최적화**: 이미지 용량을 최소화(WebP 등 권장)하고, 반드시 키워드가 포함된 `alt` 속성을 기입하여 이미지 검색 트래픽까지 확보합니다.
...
- **모바일 친화적**: 전체 트래픽의 80% 이상이 모바일에서 발생함을 인지하고, 모바일 화면에서 한눈에 들어오는 문단 길이를 유지합니다.
...
4. [ ] 사용된 모든 이미지에 `alt="설명과 키워드"` 속성이 부여되었는가?
5. [ ] 모바일 환경에서 읽기 편하도록 문단이 짧게(2~3문장) 나뉘어 있는가?
